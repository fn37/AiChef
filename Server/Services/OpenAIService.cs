...
namespace AiChef.Server.Services
{
    public class OpenAIService : IOpenAIAPI
    {
        ...
        public async Task<Recipe?> CreateRecipe(string title, List<string> ngredients)
        {
            string url = $"{_baseUrl}chat/completions";
            string systemPromt = "Your are a world-renowned chef. Create the recipe with ingredients, instructions and a summary";
            string userPrompt = $"Create a {title} recipe.";

            ChatMessage userMessage = new()
            {
                Role = "user",
                Content = $"{systemPromt} {userPrompt}"
            };

            //create request object - sent to openai
        ...
        public async Task<RecipeImage?> CreateRecipeImage(string recipeTitle)
        {
            ...
            string userPrompt = $"Create a restraunt product shot for {recipeTitle}";
